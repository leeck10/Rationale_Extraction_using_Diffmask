...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.rationale is not None:
            self.rationale_embeddings = nn.Embedding(2, config.hidden_size)
        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, rationale_ids=None):
        ...
        if rationale_ids is not None:
            rationale_embeddings = self.rationale_embeddings(rationale_ids)
            embeddings = inputs_embeds + position_embeddings + token_type_embeddings + rationale_embeddings
        else:
...
class BertModel(BertPreTrainedModel):
    ...
    Examples::
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, rationale_ids=rationale_ids
        )
